...
(function(){
  ...
  decorateRuby = function(arg$){
    ...
    if (rCjkOne.test(title)) {
      ...
      rCjkCi = new RegExp("(<a href=\"#[':~]?(" + cjk + "+)\")>\\2</a>", 'g');
      ruby = title.replace(rCjkCi, function(mat, openTag, ci, x, offset){
        var closeTag;
  ...
  function h(it){
    ...
    res = it.replace(/[\uFF0E\u2022]/g, '\u00B7').replace(/\u223C/g, '\uFF0D').replace(/\u0358/g, '\u030d').replace(/(.)\u20DD/g, "<span class='regional part-of-speech'>$1</span> ").replace(/(.)\u20DE/g, "</span><span class='part-of-speech'>$1</span><span>").replace(/(.)\u20DF/g, "<span class='specific'>$1</span>").replace(/(.)\u20E3/g, "<span class='variant'>$1</span>").replace(RegExp('<a[^<]+>' + id + '<\\/a>', 'g'), id + "").replace(/<a>([^<]+)<\/a>/g, "<a href=\"" + h + "$1\">$1</a>").replace(RegExp('(>[^<]*)' + id + '(?!</(?:h1|rb)>)', 'g'), "$1<b>" + id + "</b>").replace(/\uFFF9/g, "<span class=\"ruby" + ((typeof $ == 'function' && $('body').hasClass('lang-t')) && (typeof localStorage != 'undefined' && localStorage !== null ? localStorage.getItem('pinyin_t') : void 8) === "TL-DT" ? " parallel" : "") + "\"><span class=\"rb\"><span class=\"ruby\"><span class=\"rb\">").replace(/\uFFFA/g, '</span><br><span class="rt trs pinyin">').replace(/\uFFFB$/, '').replace(/\uFFFB/g, '</span></span></span></span><br><span class="rt mandarin">').replace(/<span class="rt mandarin">\s*<\//g, '</').replace(/(<span class="rt trs pinyin")>\s*([^<]+)/g, function(_, pre, trs){
