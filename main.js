...
(function(){
  var isCordova, isMoedictDesktop, DEBUGGING, ref$, STANDALONE, any, map, LANG, MOEID, XREFLABELOF, TITLEOF, HASHOF, STARRED, key, LRU, res$, isQuery, isDroidGap, isDeviceReady, isMobile, isApp, isWebKit, isGecko, isChrome, isPrerendered, widthIsXs, entryHistory, INDEX, XREF, CACHED, addToLru, GET, e, playing, player, seq, getEl, callLater, MOE, PUA2UNI, trs_lookup, SIMPTRAD, httpMap, LoadedScripts, amisStemmer, dedupliction, split$ = ''.split, replace$ = ''.replace, join$ = [].join;
  ...
  amisStemmer = function(w){
    ...
    inIndex = function(word){
      var myindex;
      myindex = INDEX['p'];
      return myindex.indexOf("\"" + word + "\"") >= 0;
    };
    w = w.toLowerCase();
    if (inIndex(w) || w.length <= 4) {
      return w;
    }
    for (i$ = 0, len$ = pre_suffix.length; i$ < len$; ++i$) {
      ref$ = pre_suffix[i$], pre = ref$[0], suf = ref$[1];
      if (w.startsWith(pre) && w.endsWith(suf)) {
    ...
    w = psw;
    for (i$ = 0, len$ = suffix.length; i$ < len$; ++i$) {
      suf = suffix[i$];
      if (w.endsWith(suf)) {
    ...
    return dedupliction(w);
  };
  dedupliction = function(w){
    if (w.length >= 8) {
    ...
    if (w.length >= 10 && w.slice(2, 6) === w.slice(6, 10)) {
      return w.slice(0, 6);
    }
    if (w.length >= 6 && w.slice(0, 2) === w.slice(2, 4)) {
      return w.slice(2);
    }
    return w;
  };
